/*
 * Copyright © 2017 HatioLab Inc. All rights reserved.  
 */

import MessageHub from 'message-hub-rest'

const NATURE = {
  mutable: false,
  resizable: true,
  rotatable: true,
  properties : [{
    type: 'string',
    label: 'broker',
    name: 'broker',
    property: 'broker'
  },{
    type: 'number',
    label: 'port',
    name: 'port',
    property: 'port'
  },{
    type: 'string',
    label: 'topic',
    name: 'topic',
    property: 'topic'
  },{
    type: 'string',
    label: 'Client Id',
    name: 'clientId',
    property: 'clientId'
  }]
}

const KAFKA_IMAGE = 'data:image/png;base64,';

var { RectPath, Shape } = scene

export default class Kafka extends RectPath(Shape) {
  static get image() {
    if(!Kafka._image) {
      Kafka._image = new Image()
      Kafka._image.src = KAFKA_IMAGE
    }

    return Kafka._image
  }

  ready() {
    if (!this.app.isViewMode)
      return;

    this._initMqttConnection();
  }

  get clientId() {
    return this.get('clientId')
  }

  get consumerGroupName() {
    return 'messagehub-rest-nodejs-chat-' + this.clientId
  }

  get consumerInstanceName() {
    return 'messagehub-rest-nodejs-consumer-instance-' + this.clientId
  }

  _initMqttConnection() {
    var {
      broker,
      port,
      clientId,
      topic,
      apiKey = 'DDDD'
    } = this.model

    var client = new MessageHub({
      "messagehub": [{
        "label": "messagehub",
        "credentials": {
          "api_key": apiKey,
          "kafka_rest_url": broker,
        }
      }]
    });

    //Create topic called topic with 1 partition and a retention period of 24 hours.
    client.topics.create(topic, 1, 24)
    .then(response => {
      console.log(topic, 'topic created.');
      // Set up a consumer group of the provided name.
      return client.consume(this.consumerGroupName, this.consumerInstanceName, { 'auto.offset.reset': 'largest' });
    })
    .then(response => {
      consumerInstance = response[0];
      console.log('Consumer Instance created.');
      // Set offset for current consumer instance.
      return consumerInstance.get(topic);
    })
    .fail(error => {
      console.log(error);
      stop(1);
    });

    this._client = client;
  }

  dispose() {
    super.dispose();
    this._client && this._client.disconnect();
    delete this._client;
  }

  _draw(context) {

    var {
      left,
      top,
      width,
      height
    } = this.bounds;

    context.drawImage(Kafka.image, left, top, width, height);
  }

  get nature(){
    return NATURE;
  }
}

scene.Component.register('kafka', Kafka);
